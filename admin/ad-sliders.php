...
if(isset($_POST['addHomeSlider'])) {
  if(isset($_FILES) && !empty($_FILES)) {
    ...
    if($ret['status'] && isset($ret['image_name'])) {
      ...
      if($QueryFire->insertData('ad_sliders',$arr)) {
...
if(isset($_POST['updateHomeSlider'])) {
  ...
  if($ret['status'] && isset($ret['image_name'])) {
    $arr['image_name'] = $ret['image_name'];
    $duplicate = $QueryFire->getAllData('ad_sliders',' id ='.$_REQUEST['id']);
    unlinkImage('../images/sliders/'.$duplicate[0]['image_name']);
  } else {
    $msg = 'Unable to upload file. Please try later.';
  }
  if($QueryFire->upDateTable("ad_sliders",'id ='.$_REQUEST['id'],$arr)) {
...
if(!empty($_POST['user_action'])) {
  switch($_POST['user_action']) {
    case 'delete':
        ...
        unlinkImage('../images/sliders/'.$duplicate[0]['image_name']);
        $QueryFire->deleteDataFromTable("ad_sliders",' id='.$_POST['id']);
        $msg = 'Slider deleted successfully.';
        break;
  }
}
$sliders = $QueryFire->getAllData('ad_sliders',' 1');
?>
      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">Ad Sliders</li>
          </ol>
        </div>
      </div>
    </div>
